...
1. **server 侧 warmup（已修复）**：启动后对任一源文件主动发一次 didOpen（或文档化要求客户端先调 `diagnostics`/`hover`），否则 clangd 永远不开始后台索引，符号层长期停在 `symbol-fallback-text`。→ **已实现**：`main.go` `warmUpLSP()` 在 LSP 初始化后自动打开 `compile_commands.json` 的第一个翻译单元。验证：清空索引缓存后不做任何人工预热，符号层 **6.8s** 自动就位（修复前永远停在降级态）。
2. **callers 输出无截断（已修复）**：depth=3 在 device_probe 上达 34 KB。当前仅有 depth clamp=3，未做字节级截断（search 有 12KB 总量闸，call_hierarchy 没有）。→ **已实现**：`formatCallResultsWithDepth` 增加 16KB 上限与省略计数标注。验证：同一调用 34,265 B → **16,485 B**，尾部标注 `... [truncated, 184 more results, ...]`。
3. ~~**srsRAN callers 仅 1 命中**~~（已修正压测对象）：`handle_rrc_setup_request` 由统一消息 dispatch 调用，调用链浅属真实结构。已换用热点函数 `srsran::byte_buffer::append`（全仓 1415 处引用，include/srsran/adt/byte_buffer.h:190）重测：**depth=2 → 39 个调用者 / 4.4KB；depth=3 → 1329 个 / 16.6KB**（触发 16KB 截断），depth clamp 与截断在真热点上工作正常。
